...
import static br.com.fiap.gastrosphere.core.application.utils.GastroSphereUtils.convertToAddress;
...
public class AddressController {
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(
                ...
                content = @Content(mediaType = "application/json", schema = @Schema(implementation = AddressModel.class))
            )
        }
    )
    @GetMapping
    public ResponseEntity<Page<AddressModel>> findAllAddresses(
            @RequestParam(value = "page", defaultValue = "0") int page,
            @RequestParam(value = "size", defaultValue = "10") int size,
            @RequestParam(value = "zipCode", required = false) String zipCode) {
        if (zipCode != null && !zipCode.isEmpty()) {
            logger.info("GET | {} | Iniciado busca de endereço pelo zipCode | ZipCode: {}", V1_ADDRESS, zipCode);
            Page<AddressModel> addressByZipCode = this.addressService.findAddressByZipCode(zipCode, page, size);
            ...
            Page<AddressModel> addresses = addressService.findAllAddresses(page, size);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AddressModel> findAddressById(@PathVariable("id") UUID id) {
    ...
    public ResponseEntity<UUID> createAddress(@Valid @RequestBody AddressBodyRequest addressDto) {
        ...
        AddressModel address = addressService.createAddress(convertToAddress(addressDto));
