...
public class UserTypeController {
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(
                            ...
                            content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserTypeModel.class))
                    )
            }
    )
    @GetMapping
    public ResponseEntity<Page<UserTypeModel>> findAllUserTypes(
            @RequestParam(value = "page", defaultValue = "0") int page,
            @RequestParam(value = "size", defaultValue = "10") int size) {
        logger.info("GET | {} | Iniciado busca de tipos de usuário", V1_USERTYPE);
        Page<UserTypeModel> userTypes = userTypeService.findAllUserTypes(page, size);
    ...
    @GetMapping("/{id}")
    public ResponseEntity<UserTypeModel> findUserTypeById(@PathVariable("id") UUID id) {
    ...
    public ResponseEntity<UUID> createUserType(@Valid @RequestBody UserTypeBodyRequest userTypeDto) {
        ...
        UserTypeModel createdUserType = userTypeService.createUserType(convertToUserType(userTypeDto));
